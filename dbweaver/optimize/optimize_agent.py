...
if __name__ == "__main__":
    ...
    from config import BENCHMARK, SKETCH_FIX_DIR, OPTIMIZED_CODE_DIR
    query_id = 12
    if BENCHMARK == "ssb":
    ...
    query_example = query_templates.get_query(query_id, -1)
    # split_query, split_template, pipeline_operators = decompose_query(query_example, query_template)
    ...
    # optimization_direction = "Replace Date::ExtractYear with simple comparisoms. For example, year == :year can be replaced with  days_from_civil(:year, 1, 1) < v_orderdate.days < <days_from_civil(:year+1, 1, 1). you can compute days_from_civil(:year, 1, 1) in FnInit store in global state to facilitate this."


    optimization_direction = """
    能不能消除std::string key(s.GetData(), s.GetSize());的字符串拷贝，减少内存分配和拷贝的开销
    """
    
    success, message, optimized_code, tool_calls = optimize_agent(
        cpp_code, optimization_direction, state, max_iterations=10
    )
    
    print(f"\n[Test] Success: {success}")
    print(f"[Test] Message: {message}")
    print(f"[Test] Tool calls: {len(tool_calls)}")
    
    if success and optimized_code:
        out_path = f"{OPTIMIZED_CODE_DIR}/{BENCHMARK}_q_{query_id}.cpp"
        os.makedirs(os.path.dirname(out_path), exist_ok=True)
        with open(out_path, "w") as f:
            f.write(optimized_code)
        print(f"[Test] Wrote optimized code to: {out_path}")
