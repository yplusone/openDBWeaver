    ...
    if (!l.merged) {
        {
            ...
            for (const auto &entry : l.agg_map) {
                // Copy strings into global heap
                const GroupKey &key = entry.first;
                const AggState &local_state = entry.second;
                // Copy group key into global heap
                GroupKey global_key;
                // Always re-heap into global heap to break local reference
                global_key.search_phrase = g.heap.AddString(key.search_phrase);

                auto &global_state = g.agg_map[global_key];

                // Update min_url
                if (local_state.min_url_initialized &&
                    (!global_state.min_url_initialized || local_state.min_url < global_state.min_url)) {
                    global_state.min_url = g.heap.AddString(local_state.min_url);
                    global_state.min_url_initialized = true;
                }

                // Update min_title
                if (local_state.min_title_initialized &&
                    (!global_state.min_title_initialized || local_state.min_title < global_state.min_title)) {
                    global_state.min_title = g.heap.AddString(local_state.min_title);
    ...
    if (active > 0 && merged == active) {
        {
            ...
            idx_t output_idx = 0;
            // Output only up to 10 rows (LIMIT 10)
            size_t max_output = 10;
            for (size_t i = 0; i < g.sort_state.key_buffer.size() && output_idx < max_output; ++i) {
                const GroupKey &key = g.sort_state.key_buffer[i];
                const AggState &state = g.sort_state.value_buffer[i];
                // Output: use std::string with owned string_t
                out.SetValue(0, output_idx, Value(std::string(key.search_phrase.GetData(), key.search_phrase.GetSize())));

                if (state.min_url_initialized) {
                    out.SetValue(1, output_idx, Value(std::string(state.min_url.GetData(), state.min_url.GetSize())));
                } else {
                    out.SetValue(1, output_idx, Value(LogicalType::VARCHAR));
                }
                
                if (state.min_title_initialized) {
                    out.SetValue(2, output_idx, Value(std::string(state.min_title.GetData(), state.min_title.GetSize())));
                } else {
