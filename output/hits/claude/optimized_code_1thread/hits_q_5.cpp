...
static OperatorResultType FnExecute(ExecutionContext &, TableFunctionInput &in,
    ...
    // Small stack-allocated cache for the current chunk to skip re-inserting hot values
    int64_t cache[64];
    bool cache_valid[64] = {false};

    auto push_value = [&](int64_t v) {
        const uint32_t cache_idx = static_cast<uint32_t>(static_cast<uint64_t>(v) & 63);
        if (cache_valid[cache_idx] && cache[cache_idx] == v) {
            return;
        }
        cache[cache_idx] = v;
    ...
    if (!sel->data()) {
        if (all_valid) {
            for (idx_t r = 0; r < n; r++) {
                push_value(data_ptr[r]);
            }
        } else {
            for (idx_t r = 0; r < n; r++) {
                if (valid.RowIsValid(r)) {
                    push_value(data_ptr[r]);
                }
            }
        }
        ...
        if (all_valid) {
            for (idx_t r = 0; r < n; r++) {
                push_value(data_ptr[sel->get_index(r)]);
            }
        } else {
            for (idx_t r = 0; r < n; r++) {
    ...
    if (!l.merged) {
        ...
        for (idx_t ti = 0; ti < l.touched.size(); ti++) {
            ...
            auto &gp = g.parts[p];
            {
                ...
                // Directly insert into the global hash set. absl::flat_hash_set handles range inserts efficiently.
