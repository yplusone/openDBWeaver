...
static inline int32_t ExtractMinuteFromMicros(int64_t micros) {
	if (micros >= 0) {
		const uint64_t u_micros = (uint64_t)micros;
		// M1 = ceil(2^64 / 60,000,000) = 307445734562
		const uint64_t total_minutes = (uint64_t)((unsigned __int128)u_micros * 307445734562ULL >> 64);
		// M2 = ceil(2^64 / 60) = 307445734561825861
		const uint64_t div60 = (uint64_t)((unsigned __int128)total_minutes * 307445734561825861ULL >> 64);
		return (int32_t)(total_minutes - 60 * div60);
	}
	// Fallback for negative timestamps (pre-1970)
	int64_t total_minutes = micros / 60000000LL;
	int32_t m = (int32_t)(total_minutes % 60);
	if (m < 0)
		m += 60;
	return m;
}
...
struct FnLocalState : public LocalTableFunctionState {
	...
	inline void AddOne(int64_t user_id, int32_t minute, const string_t &phrase_in, bool phrase_is_null) {
		...
		auto res = map.try_emplace(probe, 0);
		if (res.second) {
			if (!phrase_is_null && !phrase_in.IsInlined()) {
				const_cast<HashedKey &>(res.first->first).phrase = heap.AddString(phrase_in);
			}
		}
		res.first->second += 1;
	}
...
static OperatorResultType FnExecute(ExecutionContext &, TableFunctionInput &in, DataChunk &input, DataChunk &) {
	...
	if (val_uid.AllValid() && val_time.AllValid() && val_phrase.AllValid()) {
		for (idx_t ridx = 0; ridx < n; ridx++) {
			...
			const string_t phrase = phrase_data[ph_i];

			l.AddOne(uid, m, phrase, false);
		}
	} else {
		for (idx_t ridx = 0; ridx < n; ridx++) {
			...
			// SQL GROUP BY includes NULLs; but your earlier code skipped NULL uid/time.
			// Keep same behavior to match previous expectations.
			if (!val_uid.RowIsValid(uid_i))
				continue;
			if (!val_time.RowIsValid(time_i))
				continue;
...
static inline void MergeLocalIntoGlobal(FnLocalState &local, FnGlobalState &g) {
	for (auto &kv : local.map) {
		...
		auto res = g.map.try_emplace(k, 0);
		if (res.second) {
			if (!k.phrase_is_null && !k.phrase.IsInlined()) {
				const_cast<HashedKey &>(res.first->first).phrase = g.heap.AddString(k.phrase);
			}
		}
...
static OperatorFinalizeResultType FnFinalize(ExecutionContext &, TableFunctionInput &in, DataChunk &out) {
	...
	auto &l = in.local_state->Cast<FnLocalState>();
	// merge local exactly once
	if (!l.merged) {
		{
			std::lock_guard<std::mutex> guard(g.merge_lock);
			if (g.active_local_states.load(std::memory_order_relaxed) == 1) {
				// Single-threaded optimization: transfer map and heap directly to avoid O(N) merge overhead
				g.map.swap(l.map);
				g.heap.Move(l.heap);
			} else {
