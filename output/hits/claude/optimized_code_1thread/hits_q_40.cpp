...
static OperatorResultType FnExecute(ExecutionContext &, TableFunctionInput &in, DataChunk &input, DataChunk &) {
	...
	const bool all_valid =
	    u_ts.validity.AllValid() && u_se.validity.AllValid() && u_adv.validity.AllValid() &&
	    u_ref.validity.AllValid() && u_url.validity.AllValid();

	// Resolve pointers once if all types are SMALLINT (most common for hits table)
	if (t_ts == LogicalTypeId::SMALLINT && t_se == LogicalTypeId::SMALLINT && t_adv == LogicalTypeId::SMALLINT) {
		const int16_t *ts_ptr = (const int16_t *)u_ts.data;
		const int16_t *se_ptr = (const int16_t *)u_se.data;
		const int16_t *adv_ptr = (const int16_t *)u_adv.data;
		const string_t *ref_ptr = (const string_t *)u_ref.data;
		const string_t *url_ptr = (const string_t *)u_url.data;

		for (idx_t i = 0; i < input.size(); i++) {
			const idx_t ts_idx = u_ts.sel->get_index(i);
			const idx_t se_idx = u_se.sel->get_index(i);
			const idx_t adv_idx = u_adv.sel->get_index(i);
			const idx_t ref_idx = u_ref.sel->get_index(i);
			const idx_t url_idx = u_url.sel->get_index(i);

			if (!all_valid) {
				if (!u_ts.validity.RowIsValid(ts_idx) || !u_se.validity.RowIsValid(se_idx) ||
				    !u_adv.validity.RowIsValid(adv_idx) || !u_ref.validity.RowIsValid(ref_idx) ||
				    !u_url.validity.RowIsValid(url_idx)) {
					continue;
				}
			}

			const int32_t ts = (int32_t)ts_ptr[ts_idx];
			const int32_t se = (int32_t)se_ptr[se_idx];
			const int32_t adv = (int32_t)adv_ptr[adv_idx];
			const string_t referer = ref_ptr[ref_idx];
			const string_t url = url_ptr[url_idx];
	...
	// Resolve pointers once if all types are INTEGER
	if (t_ts == LogicalTypeId::INTEGER && t_se == LogicalTypeId::INTEGER && t_adv == LogicalTypeId::INTEGER) {
		const int32_t *ts_ptr = (const int32_t *)u_ts.data;
		const int32_t *se_ptr = (const int32_t *)u_se.data;
		const int32_t *adv_ptr = (const int32_t *)u_adv.data;
		const string_t *ref_ptr = (const string_t *)u_ref.data;
		const string_t *url_ptr = (const string_t *)u_url.data;
		...
		for (idx_t i = 0; i < input.size(); i++) {
			...
			const int32_t ts = ts_ptr[ts_idx];
			const int32_t se = se_ptr[se_idx];
			const int32_t adv = adv_ptr[adv_idx];
			const string_t referer = ref_ptr[ref_idx];
			const string_t url = url_ptr[url_idx];
	...
	// Fallback for mixed or other integral types
	const string_t *ref_ptr = (const string_t *)u_ref.data;
	const string_t *url_ptr = (const string_t *)u_url.data;
	for (idx_t i = 0; i < input.size(); i++) {
		const idx_t ts_idx = u_ts.sel->get_index(i);
		...
		const int32_t se = (int32_t)ReadInt64At(u_se, i, t_se);
		const int32_t adv = (int32_t)ReadInt64At(u_adv, i, t_adv);
		const string_t src = (se == 0 && adv == 0) ? ref_ptr[ref_idx] : string_t();
		l.AddOne(ts, se, adv, src, url_ptr[url_idx]);
	}
...
static inline void MergeLocalIntoGlobal(FnLocalState &local, FnGlobalState &g) {
	if (g.map.empty()) {
		g.map.swap(local.map);
		g.heap.Move(local.heap);
		return;
	}
