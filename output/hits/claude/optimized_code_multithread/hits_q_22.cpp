...
static inline bool UrlPassesFilter(const string_t &url) noexcept {
	const idx_t n = url.GetSize();
	if (n < 6) return false;
	const char *p = url.GetData();

#if defined(__SSE2__) || defined(_M_AMD64) || defined(_M_X64)
	const __m128i first_char_vec = _mm_set1_epi8('g');
	idx_t i = 0;
	// Use SIMD to scan for 'g' across 16 bytes at a time
	for (; i + 15 < n; i += 16) {
		__m128i chunk = _mm_loadu_si128(reinterpret_cast<const __m128i *>(p + i));
		__m128i cmp = _mm_cmpeq_epi8(chunk, first_char_vec);
...
#else
			uint32_t pos = static_cast<uint32_t>(__builtin_ctz(mask));
#endif
			// Check if remaining characters match "oogle"
			if (i + pos + 6 <= n) {
				if (std::memcmp(p + i + pos + 1, "oogle", 5) == 0) return true;
			}
			mask &= (mask - 1); // Clear the lowest set bit to find next match in chunk
		}
	}
	// Scalar fallback for the remainder of the string
	for (; i + 6 <= n; i++) {
		if (p[i] == 'g' && std::memcmp(p + i + 1, "oogle", 5) == 0) return true;
	}
#else
	// Scalar-only fallback for non-x86 architectures
	for (idx_t i = 0; i + 6 <= n; i++) {
		if (p[i] == 'g' && std::memcmp(p + i + 1, "oogle", 5) == 0) return true;
	}
#endif
	return false;
