...
static inline size_t HashCompositeKey(int32_t w, int32_t h) {
	return CombineHash(duckdb::Hash(w), duckdb::Hash(h));
}

// ============================================================
// Key: (WindowClientWidth, WindowClientHeight) + precomputed hash
// ============================================================
...
struct FnLocalState : public LocalTableFunctionState {
	...
	inline void AddOne(int32_t w, int32_t h, size_t hash) {
		HashedKey probe{w, h, hash};
	...
	if (all_valid) {
		for (idx_t rr = 0; rr < input.size(); rr += BATCH) {
			...
			for (idx_t k = 0; k < n; ++k) {
				...
				ws[k] = (int32_t)ReadInt64At(u_w, ridx, t_w);
				hs[k] = (int32_t)ReadInt64At(u_h, ridx, t_h);
				hashes[k] = HashCompositeKey(ws[k], hs[k]);
			}

			for (idx_t k = 0; k < n; ++k) {
				l.AddOne(ws[k], hs[k], hashes[k]);
			}
		}
	} else {
		for (idx_t rr = 0; rr < input.size(); rr += BATCH) {
			...
			for (idx_t k = 0; k < n; ++k) {
				...
				const int32_t w = (int32_t)ReadInt64At(u_w, ridx, t_w);
				const int32_t h = (int32_t)ReadInt64At(u_h, ridx, t_h);
				const size_t hash = HashCompositeKey(w, h);
...
// IMPORTANT: heap pruning must use full tie-break, not only count.
// We use order: PageViews DESC, w ASC, h ASC, hash ASC.
// ============================================================
...
struct WorstCmp {
	bool operator()(const TopRow &a, const TopRow &b) const {
		// return true if a should come after b in heap ordering;
		// by returning BetterRow(a,b), we keep "worst" on top.
...
static void EmitOffsetLimitFromMap(MapT &map_ref, DataChunk &out) {
	...
	for (auto it = map_ref.begin(); it != map_ref.end(); ++it) {
		...
		if (pq.size() < TOPK_NEED) {
			...
			// new row is better than current worst -> replace
			pq.pop();
			pq.push(row);
		}
	}
