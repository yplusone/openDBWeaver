...
struct SortRowComparator {
    bool operator()(const SortRow &a, const SortRow &b) const {
        ...
        if (a.state.count_total != b.state.count_total) return a.state.count_total > b.state.count_total;
        return false;
    }
};
...
struct SortState {
    ...
    inline void AddRow(const GroupKey &key, const AggState &state) {
        rows.push_back(SortRow{key, state});
    }
    ...
    if (active > 0 && merged == active) {
        {
            ...
            // Output the top 10 sorted results
            output_row_idx = 0;
            ...
            for (size_t i = 0; i < row_count && output_row_idx < 10; ++i) {
                ...
                const GroupKey &key = row.key;
                const AggState &state = row.state;
