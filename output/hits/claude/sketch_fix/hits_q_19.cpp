...
struct FnLocalState : public LocalTableFunctionState {
	...
	inline void AddOne(int64_t user_id, int32_t minute, const string_t &phrase_in, bool phrase_is_null) {
		...
		if (!phrase_is_null) {
			phrase = heap.AddString(phrase_in); // stable in local
		} else {
			phrase = string_t();
		}

		const size_t h = HashCompositeKey(user_id, minute, phrase_is_null, phrase);
		HashedKey probe {user_id, minute, phrase_is_null, phrase, h};
...
static inline void MergeLocalIntoGlobal(FnLocalState &local, FnGlobalState &g) {
	for (auto &kv : local.map) {
		...
		// lookup using local key (hash/content based) is fine
		...
		// materialize phrase into global heap
		...
		const size_t h = HashCompositeKey(k.user_id, k.minute, k.phrase_is_null, g_phrase);
		g.map.emplace(HashedKey {k.user_id, k.minute, k.phrase_is_null, g_phrase, h}, cnt);
	}
}
