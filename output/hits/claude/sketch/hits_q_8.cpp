...
struct SortRowComparator {
    bool operator()(const SortKeyView &a, const SortKeyView &b) const {
        // DESC order: b.cnt < a.cnt means a comes before b
...
struct SortState {
    ...
    inline void AddRow(const GroupKey &key, const AggState &state) {
        buffer.push_back(SortKeyView{state.cnt});
        keys.push_back(key);
    ...
    if (active > 0 && merged == active) {
        // Populate sort buffer
        {
            ...
            for (size_t i = 0; i < g.sort_state.buffer.size(); ++i) {
                ...
                out.SetValue(0, output_idx, Value::SMALLINT(key.AdvEngineID));
                out.SetValue(1, output_idx, Value::BIGINT(state.cnt));
                output_idx++;
            }
