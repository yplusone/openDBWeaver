...
struct SortState {
    ...
    inline void AddRow(int64_t cnt_val, const GroupKey &key) {
        buffer.push_back(SortKeyView{cnt_val});
        keys.push_back(key);
    }
    ...
    if (active > 0 && merged == active) {
        {
            ...
            for (size_t i = 0; i < g.sort_state.buffer.size(); ++i) {
                const SortKeyView &sort_key = g.sort_state.buffer[i];
                const GroupKey &key = g.sort_state.keys[i];
                
                out.SetValue(0, output_row_idx, Value::BIGINT(key.UserID));
                out.SetValue(1, output_row_idx, Value::BIGINT(sort_key.cnt));
                output_row_idx++;
            }
