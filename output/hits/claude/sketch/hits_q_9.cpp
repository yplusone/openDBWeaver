...
struct SortState {
    ...
    inline void AddRow(int64_t u_val, int32_t region_id) {
        buffer.push_back(SortKeyView{u_val});
    ...
    if (active > 0 && merged == active) {
        {
            ...
            for (size_t i = 0; i < g.sort_state.buffer.size(); ++i) {
                int32_t region_id = g.sort_state.region_ids[i];
                int64_t u = g.sort_state.buffer[i].u;
                
                out.SetValue(0, output_row_idx, Value::INTEGER(region_id));
                out.SetValue(1, output_row_idx, Value::BIGINT(u));
                output_row_idx++;
            }
