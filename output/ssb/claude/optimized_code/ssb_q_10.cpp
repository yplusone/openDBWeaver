...
static OperatorResultType FnExecute(ExecutionContext &, TableFunctionInput &in,
    ...
    VectorType lo_orderdate_vec_type = input.data[2].GetVectorType();
    ...
    if (lo_orderdate_vec_type == VectorType::CONSTANT_VECTOR) {
        // Fastest path: orderdate constant for entire chunk
        idx_t i_lo_orderdate = lo_orderdate_uvf.sel->get_index(0);
        ...
        if (date_month != target_month) {
            // Entire chunk fails date filter (skip all)
            return OperatorResultType::NEED_MORE_INPUT;
        }
        // Fast path: c_city/s_city/revenue valid checks
        if (c_city_all_valid && s_city_all_valid && lo_orderdate_all_valid && lo_revenue_all_valid) {
            for (idx_t row_idx = 0; row_idx < num_rows; ++row_idx) {
                ...
                if (c_city_str != c_city_1 && c_city_str != c_city_2) { continue; }
                if (s_city_str != s_city_1 && s_city_str != s_city_2) { continue; }
                GroupKey key;
            ...
            // Slow path for null handling (only date fast-path)
            for (idx_t row_idx = 0; row_idx < num_rows; ++row_idx) {
                idx_t i_c_city = c_city_uvf.sel->get_index(row_idx);
                idx_t i_s_city = s_city_uvf.sel->get_index(row_idx);
