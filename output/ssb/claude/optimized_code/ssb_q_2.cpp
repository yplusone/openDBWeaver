...
struct FnBindData : public FunctionData {
    ...
    explicit FnBindData(int32_t yyyymm_p, date_t start_date_p, date_t end_date_p)
        : yyyymm(yyyymm_p), start_date(start_date_p), end_date(end_date_p) {}
    
    unique_ptr<FunctionData> Copy() const override {
        return make_uniq<FnBindData>(yyyymm, start_date, end_date);
    }
...
static unique_ptr<FunctionData> FnBind(ClientContext &, TableFunctionBindInput &input,
    ...
    // Precompute start/end date range for the provided yyyymm
    int32_t year = yyyymm / 100;
    int32_t month = yyyymm % 100;
    int32_t next_year = year;
    int32_t next_month = month + 1;
    if (next_month == 13) {
        next_month = 1;
        next_year++;
    }

    date_t start_date = Date::FromDate(year, month, 1);
    date_t end_date = Date::FromDate(next_year, next_month, 1);
...
static OperatorResultType FnExecute(ExecutionContext &, TableFunctionInput &in,
    ...
    auto &bind_data = in.bind_data->Cast<FnBindData>();
    const date_t start_date = bind_data.start_date;
    const date_t end_date = bind_data.end_date;
    ...
    // ConstantVector optimization on LO_ORDERDATE
    if (input.data[0].GetVectorType() == VectorType::CONSTANT_VECTOR) {
        // Single LO_ORDERDATE value for the whole chunk
        if (!lo_orderdate_all_valid && !valid_lo_orderdate.RowIsValid(0)) {
            // Entire chunk is NULL on LO_ORDERDATE -> all rows fail predicate
            return OperatorResultType::NEED_MORE_INPUT;
        }
        const idx_t base_idx = lo_orderdate_uvf.sel->get_index(0);
        const date_t v_lo_orderdate = lo_orderdate_ptr[base_idx];

        // Range check instead of per-row year/month extraction
        if (v_lo_orderdate < start_date || v_lo_orderdate >= end_date) {
            // Date predicate fails for the single constant date -> skip whole chunk
            return OperatorResultType::NEED_MORE_INPUT;
        }


        // All rows in this chunk pass the date predicate. Only do price*discount aggregation.
        if (lo_extendedprice_all_valid && lo_discount_all_valid) {
            // Fast path: no NULLs in price/discount, simple tight loop
            for (idx_t row_idx = 0; row_idx < input.size(); ++row_idx) {
                ...
                const int64_t price = (int64_t)v_lo_extendedprice;
                const int64_t discount = (int64_t)v_lo_discount;
                l.revenue_sum += price * discount;
            }
        } else {
            // Handle potential NULLs in price/discount
            for (idx_t row_idx = 0; row_idx < input.size(); ++row_idx) {
                const idx_t i_lo_extendedprice = lo_extendedprice_uvf.sel->get_index(row_idx);
                const idx_t i_lo_discount = lo_discount_uvf.sel->get_index(row_idx);
        ...
        // All rows consumed; nothing to filter/forward
        return OperatorResultType::NEED_MORE_INPUT;
    }
    ...
    if (lo_orderdate_all_valid && lo_extendedprice_all_valid && lo_discount_all_valid) {
        ...
        for (idx_t row_idx = 0; row_idx < input.size(); ++row_idx) {
            ...
            // Apply filter using precomputed date range: start_date <= lo_orderdate < end_date
            if (v_lo_orderdate < start_date || v_lo_orderdate >= end_date) {
