            ...
            inline hugeint_t ToHugeint(__int128 acc) {
                ...
                static OperatorResultType FnExecute(ExecutionContext &context, TableFunctionInput &in,
                    ...
                    if (lo_orderdate_all_valid && s_nation_all_valid && p_category_all_valid && 
                        lo_revenue_all_valid && lo_supplycost_all_valid && p_mfgr_all_valid) {
                        ...
                        for (idx_t row_idx = 0; row_idx < input.size(); ++row_idx) {
                            ...
                            idx_t i_p_category = p_category_uvf.sel->get_index(row_idx);
                            ...
                            string_t v_s_nation = s_nation_ptr[i_s_nation];
                            string_t v_p_category = p_category_ptr[i_p_category];
                            ...
                            // Apply filter: ((EXTRACT(YEAR FROM lo_orderdate) = {year_1}) OR (EXTRACT(YEAR FROM lo_orderdate) = {year_2})) AND ((P_MFGR = {mfgr_1}) OR (P_MFGR = {mfgr_2}))
                            int32_t extracted_year = Date::ExtractYear(v_lo_orderdate);
                            bool year_match = (extracted_year == year_1) || (extracted_year == year_2);
                            bool mfgr_match = (v_p_mfgr == mfgr_1) || (v_p_mfgr == mfgr_2);
                            
                            if (!year_match || !mfgr_match) {
                                continue; // Row does not match the filter conditions
                            }
                        ...
                        for (idx_t row_idx = 0; row_idx < input.size(); ++row_idx) {
                            ...
                            if (!s_nation_all_valid && !valid_s_nation.RowIsValid(i_s_nation)) {
                                continue; // row is NULL in S_NATION → skip
                            }
                            if (!p_category_all_valid && !valid_p_category.RowIsValid(i_p_category)) {
                                continue; // row is NULL in P_CATEGORY → skip
                            }
                            ...
                            if (!lo_supplycost_all_valid && !valid_lo_supplycost.RowIsValid(i_lo_supplycost)) {
                                continue; // row is NULL in lo_supplycost → skip
                            }
                            if (!p_mfgr_all_valid && !valid_p_mfgr.RowIsValid(i_p_mfgr)) {
                                continue; // row is NULL in P_MFGR → skip
                            }
