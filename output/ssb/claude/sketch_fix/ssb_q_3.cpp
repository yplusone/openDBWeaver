...
namespace duckdb {
            ...
            struct FnBindData : public FunctionData {
                ...
                explicit FnBindData(int32_t week_p, int32_t year_p) : week(week_p), year(year_p) {}
                
                unique_ptr<FunctionData> Copy() const override { 
                    return make_uniq<FnBindData>(week, year); 
                }
...
uint8_t* lo_discount_ptr = (uint8_t*)lo_discount_uvf.data;
                    ...
                    // Output data chunk indices for rows that pass the filter
                    ...
                    if (lo_extendedprice_all_valid && lo_discount_all_valid && lo_orderdate_all_valid) {
                        ...
                        for (idx_t row_idx = 0; row_idx < input.size(); ++row_idx) {
                            ...
                            // Apply filters: 
                            // 1. (CAST(strftime('%V', lo_orderdate) AS INTEGER) = CAST({week} AS INTEGER))
                            // 2. (EXTRACT(YEAR FROM lo_orderdate) = CAST({year} AS BIGINT))
                            int32_t extracted_week = Date::ExtractISOWeekNumber(v_lo_orderdate);
                            int32_t extracted_year = Date::ExtractYear(v_lo_orderdate);
                            
                            if (extracted_week == week && extracted_year == year) {
                                // Row passes the filter, add to selection vector
                                sel_vector.set_index(output_count++, row_idx);
                                
                                // Update local accumulator: SUM(lo_extendedprice * lo_discount)
                        ...
                        for (idx_t row_idx = 0; row_idx < input.size(); ++row_idx) {
                            ...
                            if (!lo_extendedprice_all_valid && !valid_lo_extendedprice.RowIsValid(i_lo_extendedprice)) {
                                continue; // row is NULL in column lo_extendedprice → skip
                            }
                    ...
                    // Apply the selection vector to the input chunk to filter it
