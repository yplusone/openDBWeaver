...
namespace duckdb {
            ...
            static int32_t ExtractYearFromDate(date_t date) {
                auto epoch_days = int32_t(date);
                auto date_struct = Date::EpochToDate(epoch_days);
                return date_struct.year;
            }
            ...
            inline hugeint_t ToHugeint(__int128 acc) {
                ...
                static unique_ptr<FunctionData> FnBind(ClientContext &context, TableFunctionBindInput &input,
                                                    ...
                                                    vector<string> &names) {
                    // Extract parameters for the filter
                    auto year_lo = input.inputs[0].GetValue<int32_t>();
                    auto year_hi = input.inputs[1].GetValue<int32_t>();
                ...
                static void LoadInternal(ExtensionLoader &loader) {
                    TableFunction f("dbweaver", {LogicalType::INTEGER, LogicalType::INTEGER}, nullptr, FnBind, FnInit, FnInitLocal);
