...
struct FnBindData : public FunctionData {
    ...
    explicit FnBindData(string c_city_1_p, string c_city_2_p, string s_city_1_p, string s_city_2_p, string yyyymm_p) 
...
static unique_ptr<FunctionData> FnBind(ClientContext &, TableFunctionBindInput &input,
    ...
    string yyyymm = input.inputs[5].GetValue<string>();
...
static OperatorResultType FnExecute(ExecutionContext &, TableFunctionInput &in,
    ...
    // Convert yyyymm string to integer for comparison
    int target_month = std::stoi(yyyymm);
    ...
    if (c_city_all_valid && s_city_all_valid && lo_orderdate_all_valid && lo_revenue_all_valid) {
        ...
        for (idx_t row_idx = 0; row_idx < num_rows; ++row_idx) {
            ...
            auto date_struct = Date::Convert(lo_orderdate_val);
            int year = date_struct.year;
            int month = date_struct.month;
...
static void LoadInternal(ExtensionLoader &loader) {
    TableFunction f("dbweaver", {LogicalType::TABLE, LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::VARCHAR}, nullptr, FnBind, FnInit, FnInitLocal);
